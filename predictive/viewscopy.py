                ...
                if match:
                    ...
                    if params['with_actual'] == 'false':
                        ...
                        if len(msg) < 24:
                            ...
                            predictions = [{"time": f"hour{id + 1}", "value": float(i)}for id, i in enumerate(msg) if is_number(i)]
...
def training_screen(request):
    ...
    sensor_list = SettingsElement.objects.all()
    sidebar_counts = get_sidebar_counts()

    return render(request, 'training_screen.html', {'sensors': sensor_list, 'show_nav': True,**sidebar_counts})
...
def sensor_list(request):
    ...
    latest_question_list = SettingsElement.objects.values('machine_code').distinct()

    latest_question_list1 = SettingsElement.objects.all()


    paginator = Paginator(latest_question_list1, 7)  # Show 3 sensors per page
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    sidebar_counts = get_sidebar_counts()


    return render(request, 'sensordeatils/sensor_list.html', {'page_obj': page_obj,'latest_question_list':latest_question_list, 'show_nav': True,**sidebar_counts})
...
def sensor_export(request):
    ...
    for obj in SettingsElement.objects.all():
        writer.writerow([getattr(obj, field.name) for field in SettingsElement._meta.fields if field.name not in exclude_fields])
...
def anomaly_view(request):
    latest_question_list = AnomalyDataLog.objects.all()
...
def organization_list(request):
    ...
    return render(request, 'organizationsetting/organization_list.html', {'page_obj': page_obj, 'show_nav': True,**sidebar_counts})
